    ...
    for line in f:
            ...
            with open('402addresses.txt', 'a') as r:
                for command in template:
                    if command.endswith('edit'):
                        r.write('{} VPN0402-{}-{}-{}-{}'.format(command, match.group(1), match.group(2), match.group(3), match.group(4) + '\n'))
        ...
        if match583:
            with open('583addresses.txt', 'a') as r:
                for command in template:
                    if command.endswith('edit'):
                        r.write('{} VPN0583-{}-{}-{}-{}'.format(command, match583.group(1), match583.group(2), match583.group(3), match583.group(4) + '\n'))
                    elif command.endswith('subnet'):
